...
### Store IBM Quantum credentials as Camunda Secrets

Embedding credentials directly in a BPMN form is a security risk.
Store them as Camunda secrets instead:

1. In Camunda Console, go to your cluster → **Secrets**.
...
Reference them in the start form using the `{{secrets.<name>}}` syntax:
...
After configuring `application.properties` as described in the [README](../README.md#1-configure-the-connector), run:
...
c[0] = measure q[0];
```
> **Note:** The online transpilation feature was removed from IBM Quantum Platform. Thus, the quantum circuit should be pre-transpiled or a [circuit generating sidecar](use-predefined-algorithms.md) should be used.
...
Ensure the secret is created in the Camunda Console under the correct cluster.
...
Transpile the circuit for the target backend using Qiskit before submitting, or use the [IBM Quantum Algorithm Accelerator pattern](use-predefined-algorithms.md) which handles transpilation automatically.
